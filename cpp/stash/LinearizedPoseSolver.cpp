...
Eigen::Matrix3d rightDistribute(const Eigen::Matrix3d &R,
...
public:
  ...
  void setToOriginImpl() override { _estimate = Eigen::Matrix3d::Zero(); }
...
//* R1 - R0 R01
class EFNormRotation
    : public g2o::BaseBinaryEdge<9, Eigen::Matrix3d, VRelaxedRotation,
                                 VRelaxedRotation> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  void computeError() override {
    ...
    auto dr =
        static_cast<Eigen::Matrix3d>(r1->estimate() - r0->estimate() * r01);
...
public:
  ...
  void setToOriginImpl() override { _estimate = Vector6d::Zero(); }
  void oplusImpl(const double *update) override {
...
// measurement as R01
class EFNormCorrectedRotation
    : public g2o::BaseBinaryEdge<9, Eigen::Matrix3d,
                                 VCorrectedRotationWithTranslation,
                                 VCorrectedRotationWithTranslation> {
public:
  EIGEN_MAKE_ALIGNED_OPERATOR_NEW;
  ...
  void computeError() override {
    auto drt0 =
        static_cast<const VCorrectedRotationWithTranslation *>(_vertices[0])
            ->estimate();
    auto drt1 =
        static_cast<const VCorrectedRotationWithTranslation *>(_vertices[1])
            ->estimate();
    Eigen::Matrix3d R01(_measurement);
  ...
  //     _jacobianOplusXj.block<3, 3>(0, 0)  =
  //     asSkewSymmetricMatrix(R1_.row(0)); _jacobianOplusXj.block<3, 3>(0, 3)
  //     =   asSkewSymmetricMatrix(R1_.row(1)); _jacobianOplusXj.block<3, 3>(0,
  //     6)  =   asSkewSymmetricMatrix(R1_.row(2));
  // }
...
void LinearizedPoseSolver::addVertex(IDType id, const TransformType &M,
                                     bool active) {
...
void LinearizedPoseSolver::addEstimation(IDType idi, IDType idj,
                                         const MeasurementType &Mij,
                                         double weightR, double weightt) {
...
// gain
const LinearizedPoseSolver::TransformType &
LinearizedPoseSolver::gain(IDType id) const {
...
bool LinearizedPoseSolver::estimateRotation() {
  ...
  for (const auto &m : vecMeasurements_) {
    ...
    // e->computeError();
    // std::cerr<<e->chi2()<<"
    // "<<(Eigen::Matrix3d::Identity()-RBLOCK(m.Mij)).squaredNorm()<<"\n";
...
bool LinearizedPoseSolver::estimateTranslation() {
  ...
  g2o::SparseOptimizer optimizer;
  {
    ...
    // auto ptrLS  =   new
    // g2o::LinearSolverPCG<g2o::BlockSolverX::PoseMatrixType>();
...
bool LinearizedPoseSolver::estimatePose() {
  ...
  for (IDType tid = 0; tid < vecVertices_.size(); ++tid) {
    auto v6 =
        static_cast<VCorrectedRotationWithTranslation *>(optimizer.vertex(tid))
            ->estimate();
    ...
    auto &vertice = vecVertices_[tid];
    RBLOCK(vertice.M) =
        rightDistribute(RBLOCK(vertice.M), v6.block<3, 1>(0, 0));
...
bool LinearizedPoseSolver::terminateEstimation() {
  ...
  // for(const auto& v: vecVertices_){
  //     double inve =
  //     (RotationType::Identity()-RBLOCK(v.M)*RBLOCK(v.M).transpose()).norm();
